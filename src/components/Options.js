...
const Options = () => {
  ...
  const saveCharacters = (chars) => {
    const currentState = chars.reduce((obj, char) => {
      if (char.played){
  ...
  const loadCharacters = () => {
    ...
    if (cookieChars){
      ...
      if (enabled.length + played.length + disabled.length === CharacterList.length){
        handleCookieLoad(getCharactersByIds({characters: CharacterList, enabled, played, disabled}));
      }
    }
    else {
      alert('No character save (cookie) found.');
    }
  }
  return (
    <div style={themeStyle}>
      <button type="button" style={{...themeStyle.button, ...buttonStyle}} onClick={() => handleEchoClick()}>
        Echo Click
      </button>
      <button type="button" style={{...themeStyle.button, ...buttonStyle}} onClick={() => saveCharacters(characters)}>
        Save Characters as Cookie
      </button>
      <button type="button" style={{...themeStyle.button, ...buttonStyle}} onClick={() => loadCharacters()}>
