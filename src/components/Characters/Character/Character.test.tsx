...
describe('Testing Character Component', () => {
	...
	it('Checks played character', () => {
		const wrapper = shallow(
			<Character character={testChar} stateKey="played" charIndex={0} handleCharClick={context.handleCharClick} />
		);
		try {
			expect(wrapper.get(0).props.children.props.style.opacity).toEqual(0.6);
	...
	it('Checks hidden character', () => {
		const wrapper = shallow(
			<Character character={testChar} stateKey="hidden" charIndex={0} handleCharClick={context.handleCharClick} />
		);
		try {
			expect(wrapper.get(0).props.children.props.style.opacity).toEqual(0.6);
