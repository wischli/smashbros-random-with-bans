...
const Card = (props: {
  ...
  const { cookies, state, handleNextClick, handlePrevClick, displayCard, displayLoad, disableLoad, handleDisplayClick, handleCookieLoad } = props;
  ...
  return (
    <div className="card" style={cardStyle(displayCard || displayLoad)}>
      <button type="button" style={closeBtnStyle} onClick={() => (displayLoad ? disableLoad(false) : handleDisplayClick(displayCard))} className="close" />
      ...
      <div style={btnRowStyle}>
        <button type="button" style={buttonStyle(true)} onClick={() => (displayLoad ? disableLoad(false) : handlePrevClick())}>
          {displayLoad ? 'Dismiss' : 'Prev'}
        </button>
        <button type="button" style={buttonStyle(false)} onClick={() => (displayLoad ? handleCookieLoad(cookies.characters as ICState) : handleNextClick())}>
